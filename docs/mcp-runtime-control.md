...
## Tool contract (schema version 2)
...
| `get_workspace_events` | Read event cursor for incremental clients | Read-only; bounded 4,096-entry journal with truncation signaling |
...
| `execute_terminal_action` | Submit a prepared command to a tile | Internal trusted path; omitted from provider tool discovery and requires local approval |
...
`CORE_EXTENSION_API_VERSION` is 3. API-1 and API-2 companions continue to work because
