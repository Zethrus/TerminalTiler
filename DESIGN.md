...
## Source of truth
- Status: Active
- Last refreshed: 2026-07-17 (slim top bar: 32px header with ghost action buttons — transparent rest, amber-tint hover/press — 26px workspace tabs keeping the copper + amber underline identity, density profiles rescaled 38/32/28)
...
- Shape/radius/elevation: radius scale control 8 · chip 999 · card 12 · panel 14 (`profile-compact` stays squared; `workspace-summary` stays squared, with dense runtime-toolbar controls using crisp 2px corners). Elevation is soft — 1px hairline borders (`alpha(@tt_white, 0.06–0.08)`) do primary separation, shadows are secondary (e1 0.12 · e2 0.16 · e3 0.22). Alert/confirm dialog sheets use panel radius 14, a hairline outline, and the popover overlay shadow. Pill CTAs and selected-card borders retained. Top bar is a slim 32px header (comfortable 38 · compact 28): titlebar actions are ghost buttons (transparent at rest, amber-tint hover/press, 10px/600 uppercase) and workspace tabs are 26px shells that keep the copper surface + thin amber underline. The numeric scale's single source of truth is the header comment in `resources/style.css` mirrored here.
